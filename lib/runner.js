    ...
    */
    Async.series([
        function checkout(callback) {
            ...
            var err = SCM.checkout(jobConfig.scm);
            if (err) {
                callback(err, 'scm'); 
            }
            var fileOutput = '';
        ...
        function pre(callback) {
            ...
            internals.executeSync(job_id, run_id, 'pre', jobConfig.pre);
            callback(null, 'pre');
        },
        function command(callback) {

            internals.executeSync(job_id, run_id, 'command', jobConfig.command);
            callback(null, 'command');
        },
        function post(callback) {

            internals.executeSync(job_id, run_id, 'post', jobConfig.post);
            callback(null, 'post');
        },
        function notify(callback) {    
            ...
            if (err) {
                ...
                Store.saveConsoleLog(job_id, run_id, 'finish job: ' + jobConfig.name +'\n');
                Store.setRunLabel(job_id, run_id, 'success');
            }
    });
    // return the run_id so we can track it
    return run_id;
