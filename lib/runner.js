    ...
    */
    Async.series([
        ...
        function pre(callback) {
            if (jobConfig.pre) {
                var err = internals.executeSync(job_id, run_id, 'pre', jobConfig.pre);
                callback(err, 'pre'); 
            }
            else {
                callback(null, 'pre') 
        ...
        function command(callback) {
            ...
            var err = internals.executeSync(job_id, run_id, 'command', jobConfig.command);
            callback(err, 'command');
        },
        function post(callback) {
            if (jobConfig.post) {
                var err = internals.executeSync(job_id, run_id, 'post', jobConfig.post);
                callback(err, 'post');
            }
            else {
                callback(null, 'post');
